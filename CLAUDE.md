...
开始工作前必须先读取并遵守根目录 `AGENTS.md`。身份上下文、读取顺序和面试真实性红线均以 `AGENTS.md` 为统一规则源；本文件只补充 Claude 专用 Hook 行为。
...
5. 全部链接完成后，将孤儿文件路径写入 `.claude/hooks/vocab-queue.txt`
6. 写入后运行 `python ".claude/hooks/extract-vocab.py"` 触发词汇提取
