<title>day03-服务保护和分布式事务</title>

> 📂 [[微服务框架-黑马新版|课程索引]]

对应B站视频：

<readonly-block href="https://player.bilibili.com/player.html?bvid=1S142197x7&amp;amp;p=71&amp;amp;vd_source=1ff0c1b434581723cf696ccc2f59ceaa" type="iframe"></readonly-block>
...
<whiteboard token="AEcxwtoy0hreqTbEZA4ceshRnme"></whiteboard>



依次类推，整个微服务群中与购物车服务、商品服务等有调用关系的服务可能都会出现问题，最终导致整个集群不可用。

<whiteboard token="U830wlj3Ah17mVb6Vg9cNmrSn9e"></whiteboard>
...
- 知道雪崩问题产生原因及常见解决方案
- 能使用[[day08-Redis面试篇|Sentinel]]实现服务保护
- 理解分布式事务产生的原因
- 能使用Seata解决分布式事务问题
- 理解AT模式基本原理
...
![](images_day03-服务保护和分布式事务/a1e5c3bfbb35.png)
...
![](images_day03-服务保护和分布式事务/11041867f799.png)
...
<whiteboard token="MFNRwgAxsh3MExbMr2Tc4CVQnRe"></whiteboard>
...
<whiteboard token="OUuEwbjM8hpvL8bi9HTcasoanrd"></whiteboard>
...
<bookmark name="home | Sentinel" href="https://sentinelguard.io/zh-cn/"></bookmark>
...
<bookmark name="Releases · alibaba/Sentinel" href="https://github.com/alibaba/Sentinel/releases"></bookmark>

也可以直接使用课前资料提供的版本：

![](images_day03-服务保护和分布式事务/b78a0029699e.png)
...
![](images_day03-服务保护和分布式事务/dee2cbb567ea.png)
...
<bookmark name="启动配置项" href="https://github.com/alibaba/Sentinel/wiki/%E5%90%AF%E5%8A%A8%E9%85%8D%E7%BD%AE%E9%A1%B9"></bookmark>
...
![](images_day03-服务保护和分布式事务/1f11df3d1e04.png)

需要输入账号和密码，默认都是：sentinel

登录后，即可看到控制台，默认会监控sentinel-dashboard服务本身：

![](images_day03-服务保护和分布式事务/0e1030c4f40c.png)
...
![](images_day03-服务保护和分布式事务/677c1ea4d056.png)



点击簇点链路菜单，会看到下面的页面：

![](images_day03-服务保护和分布式事务/ed8cbc3c14a3.png)
...
![](images_day03-服务保护和分布式事务/a0e73b985458.png)
...
![](images_day03-服务保护和分布式事务/012432d4e9bc.png)
...
![](images_day03-服务保护和分布式事务/8856470d01ce.png)

在弹出的菜单中这样填写：

![](images_day03-服务保护和分布式事务/266009b13f1a.png)
...
![](images_day03-服务保护和分布式事务/a49f9b21b667.png)



最终监控结果如下：

![](images_day03-服务保护和分布式事务/a380d2d5ce89.png)
...
<whiteboard token="WOjhwYiWEhlEHqbIpmYcVAninEg"></whiteboard>
...
### 1.4.1.[[day01-微服务01|OpenFeign]]整合Sentinel
...
![](images_day03-服务保护和分布式事务/bd3456b40880.png)
...
![](images_day03-服务保护和分布式事务/0393eadcef98.png)

在弹出的表单中填写下面内容：

![](images_day03-服务保护和分布式事务/c6f38092548d.png)



注意，这里勾选的是并发线程数限制，也就是说这个查询功能最多使用5个线程，而不是5QPS。如果查询商品的接口每秒处理2个请求，则5个线程的实际QPS在10左右，而超出的请求自然会被拒绝。

![](images_day03-服务保护和分布式事务/da4b3b8e17d6.png)



我们利用Jemeter测试，每秒发送100个请求：

![](images_day03-服务保护和分布式事务/dbf7c37f9a42.png)

最终测试结果如下：

![](images_day03-服务保护和分布式事务/cfb98432bff5.png)
...
![](images_day03-服务保护和分布式事务/556dc779636e.png)
...
![](images_day03-服务保护和分布式事务/5bf9f9e7821d.png)
...
![](images_day03-服务保护和分布式事务/19ffb81b7a42.png)



**步骤三**：在`hm-api`模块中的`ItemClient`接口中使用`ItemClientFallbackFactory`：

![](images_day03-服务保护和分布式事务/16d4a3c84fd0.png)



重启后，再次测试，发现被限流的请求不再报错，走了降级逻辑：

![](images_day03-服务保护和分布式事务/74e09b61a0f1.png)

但是未被限流的请求延时依然很高：

![](images_day03-服务保护和分布式事务/d929da28aa67.png)
...
<whiteboard token="CnuuwL9RmhQDpHbmYXdcZCjdnUc"></whiteboard>
...
![](images_day03-服务保护和分布式事务/2a62f9edb764.png)

在弹出的表格中这样填写：

![](images_day03-服务保护和分布式事务/845dedcfd319.png)
...
![](images_day03-服务保护和分布式事务/b492808513d8.png)

在一开始一段时间是允许访问的，后来触发熔断后，查询商品服务的接口通过QPS直接为0，所有请求都被熔断了。而查询购物车的本身并没有受到影响。

此时整个购物车查询服务的平均RT影响不大：

![](images_day03-服务保护和分布式事务/0bec284ba473.png)
...
<whiteboard token="LK9rwDMt3hnCgMbYmZEcd5B6nme"></whiteboard>
...
![](images_day03-服务保护和分布式事务/c22736e79690.png)

目前有4个购物车，然结算下单，进入订单结算页面：

![](images_day03-服务保护和分布式事务/b8f4552d399c.png)

然后将购物车中某个商品的库存修改为`0`：

![](images_day03-服务保护和分布式事务/f9804da10153.png)

然后，提交订单，最终因库存不足导致下单失败：

![](images_day03-服务保护和分布式事务/15f97c7dcbf8.png)

然后我们去查看购物车列表，发现购物车数据依然被清空了，并未回滚：

![](images_day03-服务保护和分布式事务/378778614453.png)
...
<bookmark name="Seata 是什么" href="https://seata.io/zh-cn/docs/overview/what-is-seata.html"></bookmark>
...
![](images_day03-服务保护和分布式事务/89a0a9919167.png)
...
![](images_day03-服务保护和分布式事务/a35b64bdf936.png)

### 2.2.2.准备配置文件

课前资料准备了一个seata目录，其中包含了seata运行时所需要的配置文件：

![](images_day03-服务保护和分布式事务/64f5d40846db.png)

其中包含中文注释，大家可以自行阅读。

我们将整个seata文件夹拷贝到虚拟机的`/root`目录：

![](images_day03-服务保护和分布式事务/acf8041d2e56.png)



### 2.2.3.[[day04-MQ基础|Docker]]部署

需要注意，要确保[[day01-微服务01|nacos]]、mysql都在hm-net网络中。如果某个容器不再hm-net网络，可以参考下面的命令将某容器加入指定网络：
...
![](images_day03-服务保护和分布式事务/418d80c266cf.png)
...
![](images_day03-服务保护和分布式事务/563c0c9e179a.png)
...
![](images_day03-服务保护和分布式事务/bb0f51e6fe21.png)
...
![](images_day03-服务保护和分布式事务/2c51f53026d0.png)

结果：

![](images_day03-服务保护和分布式事务/ac5ae30c3d9a.png)
...
![](images_day03-服务保护和分布式事务/99f811d9f52e.png)
...
![](images_day03-服务保护和分布式事务/6800f9ae8825.png)



异常情况：

![](images_day03-服务保护和分布式事务/02bb2cd9b676.png)
...
![](images_day03-服务保护和分布式事务/d445d9247a15.png)
...
![](images_day03-服务保护和分布式事务/7f579bede3b6.png)
...
![](images_day03-服务保护和分布式事务/51c0059d6ace.png)
...
<table><colgroup><col/><col/></colgroup><thead><tr><th><h6><b>id</b></h6></th><th><h6><b>money</b></h6></th></tr></thead><tbody><tr><td>1</td><td>100</td></tr></tbody></table>
...
![](images_day03-服务保护和分布式事务/f53d01e54336.png)
...
<whiteboard token="P0AHwZDJLhtWg5bmHcLc2c2inHg"></whiteboard>
...
![](images_day03-服务保护和分布式事务/c779abcd1d7f.png)
...
![](images_day03-服务保护和分布式事务/ccc2849688c4.png)
