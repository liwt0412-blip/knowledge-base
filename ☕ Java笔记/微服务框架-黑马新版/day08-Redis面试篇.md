...
<readonly-block href="https://player.bilibili.com/player.html?bvid=1S142197x7&amp;amp;p=142&amp;amp;vd_source=1ff0c1b434581723cf696ccc2f59ceaa" type="iframe"></readonly-block>
...
- Hystix和[[day03-服务保护和分布式事务|Sentinel]]
- 限流算法
...
![](images_day08-Redis面试篇/d782e93d260f.png)
...
<table><colgroup><col/><col/><col/><col/></colgroup><thead><tr><th><h5><b>容器名</b></h5></th><th><h5><b>角色</b></h5></th><th><h5><b>IP</b></h5></th><th><h5><b>映射端口</b></h5></th></tr></thead><tbody><tr><td>r1</td><td>master</td><td>192.168.150.101</td><td>7001</td></tr><tr><td>r2</td><td>slave</td><td>192.168.150.101</td><td>7002</td></tr><tr><td>r3</td><td>slave</td><td>192.168.150.101</td><td>7003</td></tr></tbody></table>
...
![](images_day08-Redis面试篇/02c009484ba1.png)
...
![](images_day08-Redis面试篇/3e8901ee3b8d.png)
...
![](images_day08-Redis面试篇/d02452963ef2.png)

查看docker容器，发现都正常启动了：

![](images_day08-Redis面试篇/d30d2eb2e7cd.png)

由于采用的是host模式，我们看不到端口映射。不过能直接在宿主机通过ps命令查看到Redis进程：

![](images_day08-Redis面试篇/e28e034a8bfd.png)
...
![](images_day08-Redis面试篇/5a1f1ee9ce42.png)
...
![](images_day08-Redis面试篇/9da4e31f62ce.png)
...
![](images_day08-Redis面试篇/022b94e455e9.png)

再看下`r2`节点执行`replicaof`命令时的日志：

![](images_day08-Redis面试篇/f1b2521bcaf6.png)
...
![](images_day08-Redis面试篇/add532be2359.png)
...
![](images_day08-Redis面试篇/a9a6f3d4c48b.png)
...
![](images_day08-Redis面试篇/fbeb1c9fee69.png)

直到数组被填满：

![](images_day08-Redis面试篇/652223f1a713.png)

此时，如果有新的数据写入，就会覆盖数组中的旧数据。不过，旧的数据只要是绿色的，说明是已经被同步到slave的数据，即便被覆盖了也没什么影响。因为未同步的仅仅是红色部分：

![](images_day08-Redis面试篇/b7217778d12e.png)

但是，如果slave出现网络阻塞，导致master的`offset`远远超过了slave的`offset`： 

![](images_day08-Redis面试篇/190129451b5c.png)

如果master继续写入新数据，master的`offset`就会覆盖`repl_baklog`中旧的数据，直到将slave现在的`offset`也覆盖：

![](images_day08-Redis面试篇/6f135dec2c9b.png)
...
![](images_day08-Redis面试篇/b1fdc5995341.png)
...
![](images_day08-Redis面试篇/04c53c575b91.png)
...
![](images_day08-Redis面试篇/75ec2dd905ce.png)
...
<bookmark name="High availability with Redis Sentinel" href="https://redis.io/docs/management/sentinel/#replica-selection-and-priority"></bookmark>
...
![](images_day08-Redis面试篇/8c18ce56e2c4.png)
...
![](images_day08-Redis面试篇/fa3ca71770d2.png)
...
![](images_day08-Redis面试篇/de55715b7c50.png)
...
![](images_day08-Redis面试篇/9aa82c89867a.png)
...
![](images_day08-Redis面试篇/f94e7e48247d.png)
...
![](images_day08-Redis面试篇/2835220b798e.png)



计划部署的节点信息如下：

<sheet sheet-id="kofYek" token="CpEbsnvLWh7hXCtMcE7crYiln4d"></sheet>
...
![](images_day08-Redis面试篇/d3a3b08da796.png)
...
![](images_day08-Redis面试篇/58dd4b6b4cfa.png)
...
![](images_day08-Redis面试篇/076d40f41e6d.png)
...
<bookmark name="Scale with Redis Cluster" href="https://redis.io/docs/management/scaling/#redis-cluster-101"></bookmark>



在Redis集群中，共有16384个`hash slots`，集群中的每一个master节点都会分配一定数量的`hash slots`。具体的分配在集群创建时就已经指定了：

![](images_day08-Redis面试篇/e76e4c70f07a.png)
...
![](images_day08-Redis面试篇/df5b03aa5413.png)
...
![](images_day08-Redis面试篇/3d617c888d17.png)
...
![](images_day08-Redis面试篇/d5f5c5b990cd.png)
...
![](images_day08-Redis面试篇/4591c776f8df.png)

过了一段时间后，7002原本的小弟7006变成了`master`：

![](images_day08-Redis面试篇/da6fe3071a2e.png)
...
![](images_day08-Redis面试篇/c46ac00ef1ba.png)
...
<table><colgroup><col/><col/><col/></colgroup><thead><tr><th><h5><b>编号</b></h5></th><th><h5><b>编码方式</b></h5></th><th><h5><b>说明</b></h5></th></tr></thead><tbody><tr><td>0</td><td>OBJ_ENCODING_RAW</td><td>raw编码动态字符串</td></tr><tr><td>1</td><td>OBJ_ENCODING_INT</td><td>long类型的整数的字符串</td></tr><tr><td>2</td><td>OBJ_ENCODING_HT</td><td>hash表（也叫dict）</td></tr><tr><td>3</td><td>OBJ_ENCODING_ZIPMAP</td><td>已废弃</td></tr><tr><td>4</td><td>OBJ_ENCODING_LINKEDLIST</td><td>双端链表</td></tr><tr><td>5</td><td>OBJ_ENCODING_ZIPLIST</td><td>压缩列表</td></tr><tr><td>6</td><td>OBJ_ENCODING_INTSET</td><td>整数集合</td></tr><tr><td>7</td><td>OBJ_ENCODING_SKIPLIST</td><td>跳表</td></tr><tr><td>8</td><td>OBJ_ENCODING_EMBSTR</td><td>embstr编码的动态字符串</td></tr><tr><td>9</td><td>OBJ_ENCODING_QUICKLIST</td><td>快速列表</td></tr><tr><td>10</td><td>OBJ_ENCODING_STREAM</td><td>Stream流</td></tr><tr><td>11</td><td>OBJ_ENCODING_LISTPACK</td><td>紧凑列表</td></tr></tbody></table>

Redis中的5种不同的数据类型采用的底层数据结构和编码方式如下：

<table><colgroup><col/><col/></colgroup><thead><tr><th><h5><b>数据类型</b></h5></th><th><h5><b>编码方式</b></h5></th></tr></thead><tbody><tr><td>STRING</td><td><code>int</code>、<code>embstr</code>、<code>raw</code></td></tr><tr><td>LIST</td><td><code>LinkedList和ZipList</code>(3.2以前)、<code>QuickList</code>（3.2以后）</td></tr><tr><td>SET</td><td><code>intset</code>、<code>HT</code></td></tr><tr><td>ZSET</td><td><code>ZipList</code>（7.0以前）、<code>Listpack</code>（7.0以后）、<code>HT</code>、<code>SkipList</code></td></tr><tr><td>HASH</td><td><code>ZipList</code>（7.0以前）、<code>Listpack</code>（7.0以后）、<code>HT</code></td></tr></tbody></table>

这些数据类型比较复杂，我们**重点讲解几个面试会问**的，其它的大家可以查看黑马程序员发布的Redis专业课程：

<readonly-block href="https://player.bilibili.com/player.html?bvid=BV1cr4y1671t&amp;amp;p=145&amp;amp;page=145&amp;amp;autoplay=0" type="iframe"></readonly-block>
...
![](images_day08-Redis面试篇/0653075c28d5.png)

我们可以看到1号元素就有指向3、5、10的多个指针，查询时就可以跳跃查找。例如我们要找大小为14的元素，查找的流程是这样的：

![](images_day08-Redis面试篇/7ba7d25fba54.png)
...
![](images_day08-Redis面试篇/41944698869b.png)
...
![](images_day08-Redis面试篇/8750e02fa2d0.png)
...
![](images_day08-Redis面试篇/967838c37396.png)
...
![](images_day08-Redis面试篇/dfea5ae958c6.png)
...
![](images_day08-Redis面试篇/bcabb3183ff2.png)

**异常情况**

![](images_day08-Redis面试篇/21038a5d412f.png)
...
![](images_day08-Redis面试篇/4fa125a53b76.png)

**异常情况**

![](images_day08-Redis面试篇/7f3a73c2416f.png)
...
![](images_day08-Redis面试篇/c1257ac133d3.png)
...
![](images_day08-Redis面试篇/bbeeca524b2d.png)
...
![](images_day08-Redis面试篇/a3b4f24cdd28.png)
...
![](images_day08-Redis面试篇/6410d89c7626.png)
...
![](images_day08-Redis面试篇/0533bd0e2502.png)
...
![](images_day08-Redis面试篇/0e3b4aa62f5e.png)
...
![](images_day08-Redis面试篇/c85bbc933df3.png)



逻辑过期的思路如图：

![](images_day08-Redis面试篇/aadb3a618187.png)
