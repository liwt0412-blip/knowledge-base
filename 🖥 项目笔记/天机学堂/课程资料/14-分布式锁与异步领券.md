在昨天的学习中，我们已经实现了[[13-领取优惠券|领取优惠券]]的功能，并且解决了多线程下的券超发的并发安全问题。不过，之前我们考虑的是单机模式下的多线程问题，解决的思路是基于Synchronized锁。
...
- 能实现基于注解的分布式锁
- 能利用MQ解决高并发写性能问题
- 了解Redis的[[14-分布式锁与异步领券|LUA]]脚本的作用
...
<bookmark name="04.027-Monitor-工作原理_哔哩哔哩_bilibili" href="https://www.bilibili.com/video/BV16J411h7Rd?p=76&amp;vd_source=1ff0c1b434581723cf696ccc2f59ceaa"></bookmark>
...
![](14-分布式锁与异步领券/5e7992777d83.png)

利用RedisLock替代原本的synchronized锁：

![](14-分布式锁与异步领券/58195402ab34.png)
...
![](14-分布式锁与异步领券/f1d293d76f71.png)

综上，分布式锁的实现逻辑就变化了：

![](14-分布式锁与异步领券/716e0240ea53.png)
...
<bookmark name="实战篇-19.分布式锁-Redisson的可重入锁原理_哔哩哔哩_bilibili" href="https://www.bilibili.com/video/BV1cr4y1671t?p=66&amp;vd_source=1ff0c1b434581723cf696ccc2f59ceaa"></bookmark>
...
<bookmark name="redisson.org" href="https://redisson.org/"></bookmark>



### 1.4.1.介绍

![](14-分布式锁与异步领券/6f09d2563b08.png)
...
![](14-分布式锁与异步领券/641b76296244.png)
</column>
<column width-ratio="0.500000">
![](14-分布式锁与异步领券/7e8e869a8acb.png)
</column>
</grid>
...
![](14-分布式锁与异步领券/87fc01d9968f.png)

其中的一些关键配置：

![](14-分布式锁与异步领券/2f1e609183a0.png)

![](14-分布式锁与异步领券/f572d3a7baa2.png)
...
![](14-分布式锁与异步领券/b6dfc458b39b.png)



此时，在业务中，就能通过注解来指定自己要用的锁类型了：

![](14-分布式锁与异步领券/11b897c64e33.png)
...
![](14-分布式锁与异步领券/2d0419879fd1.png)



最后，修改切面代码，基于用户选择的策略来处理：

![](14-分布式锁与异步领券/1c5ec43593eb.png)



这个时候，我们就可以在使用锁的时候自由选择锁类型、锁策略了：

![](14-分布式锁与异步领券/5785463b6429.png)
...
<bookmark name="6. Spring Expression Language (SpEL)" href="https://docs.spring.io/spring-framework/docs/3.0.x/reference/expressions.html"></bookmark>

中文文档：

<bookmark name="8. Spring 表 达 式 语 言 (SpEL)" href="http://itmyhome.com/spring/expressions.html"></bookmark>



首先，在使用锁注解时，锁名称可以利用SPEL表达式，例如我们指定锁名称中要包含参数中的用户id，则可以这样写：

![](14-分布式锁与异步领券/c9b734f181bd.png)

而如果是通过UserContext.getUser()获取，则可以利用下面的语法：

![](14-分布式锁与异步领券/b211f5e5c06a.png)
...
![](14-分布式锁与异步领券/5450179f2f79.png)

可以看到，其中有大量的对**写数据库**的操作，而且过程中还加锁保证线程安全，接口串行执行导致性能有很大影响。必须想办法提高效率。

那么对于这种**高并发**写业务，同时逻辑比较长且复杂的业务，该如何做优化呢？
...
![](14-分布式锁与异步领券/53f31a770d57.png)

- 合并写：

![](14-分布式锁与异步领券/b56f122819d3.png)
...
<table><colgroup><col/><col/><col/></colgroup><thead><tr><th vertical-align="middle"><b>KEY(couponId)</b></th><th vertical-align="middle"><b>field</b></th><th vertical-align="middle"><b>value</b></th></tr></thead><tbody><tr><td rowspan="4" vertical-align="middle">couponId:10</td><td vertical-align="middle">issueBeginTime</td><td vertical-align="middle">20230327</td></tr><tr><td vertical-align="middle">issueEndTime</td><td vertical-align="middle">20230501</td></tr><tr><td vertical-align="middle">totalNum</td><td vertical-align="middle">100</td></tr><tr><td vertical-align="middle">userLimit</td><td vertical-align="middle">1</td></tr><tr><td rowspan="4" vertical-align="middle">couponId:20</td><td vertical-align="middle">issueBeginTime</td><td vertical-align="middle">20230827</td></tr><tr><td vertical-align="middle">issueEndTime</td><td vertical-align="middle">20230901</td></tr><tr><td vertical-align="middle">totalNum</td><td vertical-align="middle">200</td></tr><tr><td vertical-align="middle">userLimit</td><td vertical-align="middle">2</td></tr></tbody></table>
...
<table><colgroup><col/><col/><col/></colgroup><thead><tr><th vertical-align="middle"><b>KEY(couponId)</b></th><th vertical-align="middle"><b>field(userId)</b></th><th vertical-align="middle"><b>value(count)</b></th></tr></thead><tbody><tr><td rowspan="4" vertical-align="middle">couponId:10</td><td vertical-align="middle">uid:110</td><td vertical-align="middle">1</td></tr><tr><td vertical-align="middle">uid:120</td><td vertical-align="middle">1</td></tr><tr><td vertical-align="middle">uid:130</td><td vertical-align="middle">1</td></tr><tr><td vertical-align="middle">uid:140</td><td vertical-align="middle">1</td></tr></tbody></table>
...
![](14-分布式锁与异步领券/07d93d91a69b.png)

内容如下：

![](14-分布式锁与异步领券/eb0f24b25738.png)
...
<table><colgroup><col/><col/><col/><col/></colgroup><thead><tr><th vertical-align="middle"><b>参数</b></th><th colspan="3" vertical-align="middle"><b>说明</b></th></tr></thead><tbody><tr><td vertical-align="middle"><b>Exchange</b></td><td colspan="3" vertical-align="middle">promotion.topic</td></tr><tr><td vertical-align="middle"><b>Routing-Key</b></td><td colspan="3" vertical-align="middle">coupon:receive</td></tr><tr><td rowspan="3" vertical-align="middle"><b>Message</b></td><td vertical-align="middle">参数名</td><td vertical-align="middle">类型</td><td vertical-align="middle">说明</td></tr><tr><td vertical-align="middle">userId</td><td vertical-align="middle">Long</td><td vertical-align="middle">用户id</td></tr><tr><td vertical-align="middle">couponId</td><td vertical-align="middle">Long</td><td vertical-align="middle">优惠券id</td></tr></tbody></table>



我们现在`tj-common`的`MqConstants`中将`Exchange`和`RoutingKey`定义出来：

![](14-分布式锁与异步领券/1baf8322da98.png)



然后在tj-promotion中定义MQ消息传输的DTO（课前资料已经提供）：

![](14-分布式锁与异步领券/b01dc19f1828.png)
...
- 改造兑换优惠券的功能，利用Redis完成资格校验，然后发送MQ消息（消息体中要增加传递**兑换码的序列号**）
...
<bookmark name="Lua 5.4 Reference Manual - contents" href="https://www.lua.org/manual/5.4/"></bookmark>

中文教程：

<bookmark name="Lua 教程_w3cschool" href="https://www.w3cschool.cn/lua/"></bookmark>
...
<bookmark name="Redis Lua API reference" href="https://redis.io/docs/manual/programmability/lua-api/"></bookmark>

也可以参考一些播客：

<bookmark name="快速入门Redis调用Lua脚本及使用场景介绍" href="https://segmentfault.com/a/1190000039287716"></bookmark>

或者黑马的Redis课程：

<bookmark name="实战篇-15.分布式锁-Lua脚本解决多条命令原子性问题_哔哩哔哩_bilibili" href="https://www.bilibili.com/video/BV1cr4y1671t/?p=62&amp;spm_id_from=pageDriver&amp;vd_source=1ff0c1b434581723cf696ccc2f59ceaa"></bookmark>
