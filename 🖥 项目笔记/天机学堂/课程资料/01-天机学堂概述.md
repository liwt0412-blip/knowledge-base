<callout emoji="🏖️"><p><b>号外号外！现在点击</b><b><cite doc-id="MnIqwghIsitrrVkeasjcmWA5nhB" file-type="wiki" title="黑马逆袭之路！！！" type="doc"></cite></b><b>，添加黑马老师即可领取多套试学课程。还可以预约1V1职业规划咨询帮你找准定位、不再盲目。赶快行动起来吧！</b></p></callout>
...
![](01-天机学堂概述/af08936f61a3.png)



职业教育的市场规模持续增长，增长率保持在12%以上，总规模即将突破万亿，可见职业教育前景大好。职业教育培训分为有学历和非学历两大类：

<readonly-block token="CBSZbeZaDml7eEnCaN9cFxMgnPU" type="mindnote"></readonly-block>
...
![](01-天机学堂概述/4a165b1dd1d5.jpg)



## 2.4.技术架构

![](01-天机学堂概述/2710f9d38208.png)
...
<whiteboard token="AWqjwjRALho7yPblCf8cdeidnyf"></whiteboard>
...
![](01-天机学堂概述/cf77fd28a4d8.png)

- 然后，点击《管理Web钩子》菜单，进入页面后点击钩子后面的修改按钮：

![](01-天机学堂概述/6d62a839bdb8.png)

进入页面后，向下滚动，点击测试推送按钮：

![](01-天机学堂概述/38c842a91c92.png)

然后回到jenkins页面，会发现已经触发了`tjxt-dev-build`的自动编译：

![](01-天机学堂概述/c885c0fab102.png)
...
![](01-天机学堂概述/9af12e4f7751.png)

此时访问 [http://www.tianji.com](http://www.tianji.com) (jack/123    rose/123456)即可看到用户端页面：

![](01-天机学堂概述/12da62a7d9ed.png)

此时访问 [http://manage.tianji.com](http://manage.tianji.com) 即可看到管理端页面：

![](01-天机学堂概述/9e462f281f4e.png)

如果想要知道微服务具备哪些API接口，可以访问网关中的swagger页面，路径如下：

http://api.tianji.com/doc.html

![](01-天机学堂概述/a14f3c906ab3.png)
...
查看Git私服的代码：[http://git.tianji.com/tjxt/tianji](http://git.tianji.com/tjxt/tianji) ：

![](01-天机学堂概述/717aba416c73.png)
...
<whiteboard token="RfyUwo57uhemV9bZYidceJcXnff"></whiteboard>



对应到我们项目中每个模块及功能如下：

![](01-天机学堂概述/31d53c55d438.png)



当我们要创建新的微服务时，也必须以`tjxt`为父工程，创建一个子`module`. 例如交易微服务：

![](01-天机学堂概述/d1cb342b6895.png)

微服务`module`中如果有对外暴露的Feign接口，需要定义到`tj-api`模块中：

![](01-天机学堂概述/80e8b5ca54d8.png)
...
![](01-天机学堂概述/3b50c5071c9f.png)

不过，这种模式是不被Spring推荐的，Spring推荐的是基于构造函数注入，像这样：

![](01-天机学堂概述/5cc1a08cc287.png)
...
![](01-天机学堂概述/957b9dddabd3.png)

这样一来，不管需要注入的字段再多，我们也只需要一个注解搞定：

![](01-天机学堂概述/132c1a205778.png)
...
![](01-天机学堂概述/13690af01e99.png)

我们切换到萝丝用户登录（rose/123456），再次删除一个订单：

![](01-天机学堂概述/b8eeb94d9438.png)
...
<whiteboard token="ZLdaw1RruhNyjdb4nibcyhl5nUe"></whiteboard>
...
![](01-天机学堂概述/e20937c6eaf4.png)

跟入service代码：

![](01-天机学堂概述/42a5361c6c35.png)
...
![](01-天机学堂概述/80ab4aa493aa.png)

然后添加一个新的启动项：

![](01-天机学堂概述/197e0ca28698.png)

在新建的`Configuration`中填写信息：

![](01-天机学堂概述/f16d97a3a702.png)

 此时，就可以在启动项中看到我们配置的远程调试项目了：

![](01-天机学堂概述/e49926f64e80.png)
...
![](01-天机学堂概述/8a5874bf2718.png)

部署完成后，可以看到`tj-trade`多暴露了一个5005端口，就是远程调试的端口了：

![](01-天机学堂概述/00318d4c359e.png)
...
![](01-天机学堂概述/ce14ad95f732.png)

接下来，提交代码：

![](01-天机学堂概述/8a6567162999.png)

然后切换会Dev分支，并将hotfix-delete-order-error分支合并到dev分支，然后删除：

![](01-天机学堂概述/78ce4e0d904d.png)
...
![](01-天机学堂概述/a8653b82e36c.png)

由于删除订单时需要对登录用户做校验，因此需要先设置用户id的全局参数：

<grid>
<column width-ratio="0.500000">
![](01-天机学堂概述/af7917a5f97c.png)
</column>
<column width-ratio="0.500000">
![](01-天机学堂概述/4eec611f4e55.png)
</column>
</grid>
...
![](01-天机学堂概述/7d9c428560f8.png)

当用户id正确时，删除成功：

![](01-天机学堂概述/92c02954c116.png)
...
![](01-天机学堂概述/38bd8379e626.png)

修改其中的配置。

第一个是哪些分支变化以后触发构建：

![](01-天机学堂概述/1a1e207f13e7.png)

第二个是构建时基于哪个分支构建：

![](01-天机学堂概述/0a557144c055.png)

然后选择提交dev分支，并push到远端仓库：

![](01-天机学堂概述/e25a54094828.png)

![](01-天机学堂概述/4e8757177ce0.png)



然后到控制台，重新构建`tj-trade`服务：

![](01-天机学堂概述/dd992634912b.png)

将本地服务停止，修改nacos中的虚拟机中的tj-trade实例权重为1：

![](01-天机学堂概述/ab7cf588083f.png)
