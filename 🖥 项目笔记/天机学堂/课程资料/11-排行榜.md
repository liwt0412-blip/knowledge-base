...
<bookmark name="Commands" href="https://redis.io/commands/?group=sorted-set"></bookmark>
...
<whiteboard token="KHdJwWw65h4qRbbJpmMcA7Qynhf"></whiteboard>
...
<table><colgroup><col/><col/></colgroup><tbody><tr><td vertical-align="middle">接口说明</td><td vertical-align="middle">查询指定赛季的积分排行榜以及当前用户的积分和排名信息</td></tr><tr><td vertical-align="middle">请求方式</td><td vertical-align="middle">GET</td></tr><tr><td vertical-align="middle">请求路径</td><td vertical-align="middle">/boards</td></tr><tr><td vertical-align="middle">请求参数</td><td vertical-align="middle"><ul><li>分页参数，例如PageNo、PageSize</li><li>赛季id，为空或0时，代表查询当前赛季。否则就是查询历史赛季</li></ul></td></tr><tr><td vertical-align="middle">返回值</td><td vertical-align="middle"><pre lang="JSON"><code>{<br/>    "rank": 8, // 当前用户的排名<br/>    "points": 21, // 当前用户的积分值<br/>    [<br/>        {<br/>            "rank": 1, // 排名<br/>            "points": 81, // 积分值<br/>            "name": "Jack" // 姓名<br/>        }，<br/>        {<br/>            "rank": 2, // 排名<br/>            "points": 74, // 积分值<br/>            "name": "Rose" // 姓名<br/>        }<br/>    ]<br/>}</code></pre></td></tr></tbody></table>
...
![](11-排行榜/7087ae5661f1.png)

然后是分页VO实体、分页条目VO实体：

![](11-排行榜/e8d0fad58574.png)
...
<whiteboard token="DNrQwDAAnh1uDzbQ8DHcvq61nWb"></whiteboard>
...
<bookmark name="MySQL分区表 - wenxuehai - 博客园" href="https://www.cnblogs.com/wenxuehai/p/15901779.html"></bookmark>
...
![](11-排行榜/f062abde77f5.png)



同时，表中的字段少了2个（rank、season），因此我们需要修改对应的实体类：

![](11-排行榜/414dadc4d792.png)
...
- [[11-排行榜|XXL-JOB]]：开源免费，功能虽然不如PowerJob，不过目前市场占比最高，稳定性有保证。
...
<bookmark name="分布式任务调度平台XXL-JOB" href="https://www.xuxueli.com/xxl-job/"></bookmark>

XXL-JOB的运行原理和架构如图：

![](11-排行榜/ec052953f359.png)
...
![](11-排行榜/2bd496067790.png)

最终XXL-JOB的表结构如下：

![](11-排行榜/8e0edfbb9d30.png)
...
![](11-排行榜/124c751e0023.png)

配置中的关键属性都已经在Nacos中共享了：

![](11-排行榜/74d69d754088.png)
...
![](11-排行榜/2b5052d33091.png)

在弹出的窗口中填写信息：

![](11-排行榜/b1e76479a04b.png)

等待一段时间，会发现`learning-service`已经成功注册了：

![](11-排行榜/c5cd3d2fe292.png)
...
![](11-排行榜/16dd8ecb1a0e.png)

在弹出表单中，填写任务调度信息：

![](11-排行榜/36d6fdd0ed0b.png)
...
![](11-排行榜/73993901db77.png)

然后在弹出的窗口中，直接点保存即可执行：

![](11-排行榜/4482b2c6c200.png)

注意，如果是分片广播模式， 这里还可以填写一些任务参数。

然后在调度日志中，可以看到执行成功的日志信息：

![](11-排行榜/f8a16da32b54.png)
...
<whiteboard token="XLfgwoGVfhkfCUbZ6Y9c4KcDnhg"></whiteboard>
...
<bookmark name="动态表名插件 | MyBatis-Plus" href="https://baomidou.com/pages/2a45ff/#dynamictablenameinnerinterceptor"></bookmark>

插件的部分源码如下：

![](11-排行榜/296202c60e3a.png)

可见表名称动态获取就是依赖于tableNameHandlerMapping中的具体的TableNameHandler，这个Map如图：

![](11-排行榜/dab2d3056ae6.png)
...
<whiteboard token="DlttwbxkNhql0abeR6Nc8zPXnLM"></whiteboard>
...
<whiteboard token="VqKCwOCOehxVB8bbeivcdMPHnKd"></whiteboard>



我们在`tj-learning`的`com.tianji.learning.utils`包下定义一个传递表名称的工具：

![](11-排行榜/75b46ed06d90.png)
...
![](11-排行榜/e7815368dda3.png)

修改其中的拦截器配置：

![](11-排行榜/fe17c3e70630.png)

<callout emoji="❗">
**注意**：
- 由于`DynamicTableNameInnerInterceptor`并不是每一个微服务都用了，所以这里加入了@Autowired(required= false)，避免未定义该拦截器的微服务报错。
- MybatisPlus的插件定义顺序非常重要，必须按照一定的顺序来定义。参考：https://baomidou.com/plugins/
</callout>
...
<whiteboard token="MnOUwjnyqhyrQibxnCScgAcDnRH"></whiteboard>
...
![](11-排行榜/9564c18de4b3.png)

然后是清理Redis的任务：

![](11-排行榜/10fc5d81eb9e.png)



接下来，回到任务管理页面，会看到3个任务都添加成功，并且每个任务都有自己的ID：

![](11-排行榜/7db6f2c85d92.png)
...
![](11-排行榜/e0051a799d09.png)

然后在子任务中，填写持久化榜单数据任务的id，本例中是12：

![](11-排行榜/72a730afe320.png)

保存。

然后点击持久化榜单数据任务后面的操作，编辑：

![](11-排行榜/f19619b85d36.png)

然后在子任务一栏，填写清理Redis中的历史榜单的任务id，本例中是13：

![](11-排行榜/d588b07d271c.png)
